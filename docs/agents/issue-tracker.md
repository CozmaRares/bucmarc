...
- QA work goes under `.scratch/_qa/<slug>/`
...
Run `bun run qa:report` to print a Markdown list of QA reports under `.scratch/_qa/**/REPORT.md` marked with the canonical `needs-triage` status.

## When a skill says "publish to the issue tracker"

Create a new file under `.scratch/<feature-slug>/` (creating the directory if needed).

For QA sessions, create or update files under `.scratch/_qa/<slug>/`.
